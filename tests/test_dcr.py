...
from app.dcr import DcrRegistrationError, DcrResult, register_client


@pytest.fixture(autouse=True)
def _dcr_endpoint():
    with patch("app.dcr.settings.DCR_REGISTRATION_ENDPOINT", "https://provider.example.com/register"):
        yield
...
class TestRegisterClient:
    async def test_returns_dcr_result_on_success(self):
        result = await register_client(_METADATA, _transport=_transport(201, _DCR_RESPONSE))
        assert isinstance(result, DcrResult)
    ...
    async def test_raises_on_provider_401(self):
        with pytest.raises(DcrRegistrationError, match="401"):
            await register_client(_METADATA, _transport=_transport(401, {"error": "unauthorized"}))

    async def test_raises_on_provider_500(self):
        with pytest.raises(DcrRegistrationError, match="500"):
            await register_client(_METADATA, _transport=_transport(500, {"error": "server error"}))
    ...
    async def test_posts_with_correct_method(self):
        ...
        await register_client(_METADATA, _transport=httpx.MockTransport(handler))
