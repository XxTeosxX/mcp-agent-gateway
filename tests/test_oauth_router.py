...
from app.client_metadata import ClientMetadata, ClientMetadataFetchError, ClientMetadataValidationError
from app.config import settings
from app.dcr import DcrRegistrationError, DcrResult

_METADATA = ClientMetadata(
    redirect_uris=["https://myapp.com/callback"],
    client_name="My MCP App",
)
_DCR_RESULT = DcrResult(client_id="kc-abc", client_secret="secret")
...
class TestOAuthAuthorize:
    def test_url_client_id_redirects_to_as(self, client: TestClient) -> None:
        with (
            patch("app.routers.oauth.fetch_client_metadata", new=AsyncMock(return_value=_METADATA)),
            patch("app.routers.oauth.register_client", new=AsyncMock(return_value=_DCR_RESULT)),
            patch("app.routers.oauth._get_redis", return_value=_fake_redis()),
        ):
            resp = client.get(
    ...
    def test_cache_hit_skips_fetch_and_dcr(self, client: TestClient) -> None:
        ...
        with (
            patch("app.routers.oauth.fetch_client_metadata", new=fetch_mock),
            patch("app.routers.oauth.register_client", new=dcr_mock),
            patch("app.routers.oauth.client_registry.get", new=fake_get),
            patch("app.routers.oauth.client_registry.set", new=fake_set),
        ):
    ...
    def test_metadata_fetch_failure_returns_invalid_client(self, client: TestClient) -> None:
        with patch(
            "app.routers.oauth.fetch_client_metadata",
    ...
    def test_redirect_uri_not_in_metadata_returns_invalid_request(self, client: TestClient) -> None:
        ...
        with (
            patch("app.routers.oauth.fetch_client_metadata", new=AsyncMock(return_value=metadata_other)),
            patch("app.routers.oauth._get_redis", return_value=_fake_redis()),
        ):
            resp = client.get(
                _authorize_url(
    ...
    def test_dcr_failure_returns_server_error(self, client: TestClient) -> None:
        with (
            patch("app.routers.oauth.fetch_client_metadata", new=AsyncMock(return_value=_METADATA)),
            patch(
                "app.routers.oauth.register_client",
